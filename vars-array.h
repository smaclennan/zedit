...
struct avar Vars[] = {
	{ "add-nl",		FLAG,		{1}
	  DOCSTR("If this flag variable is set, the editor will insure that "
		 "all saved files end in a Newline. The default is on.")
	},
	{ "backup",		FLAG,		{0}
	  DOCSTR("If this flag variable is set, a backup file is created when "
		 "a file is written. The default is off.")
	},
	{ "c-extends",		STRING,		{0}
	  DOCSTR("This variable defines up to 8 extensions that turn on C "
		 "Mode. The extensions are seperated with the colon (:).")
	},
	{ "c-tabs",		DECIMAL,	{8}
	  DOCSTR("A tab is a single character that is displayed as one or more "
	...
	{ "exact",		FLAG,		{1}
	  DOCSTR("If Exact is set, all searches and replaces are case "
		 "sensitive. This means that the search string 'string' will "
		 "only match 'string', not 'STRING', or 'String'. The default "
		 "is on.")
	},
	{ "expand-paths",	FLAG,		{1}
	  DOCSTR("If ExpandPaths is set, all file names are expanded to a full "
	...
	{ "fill-ch-width",	DECIMAL,	{72}
	  DOCSTR("This is the column at which the Text mode commands wrap the "
		 "right margin. It is also used by the Center Line command. "
		 "The default is 72.")
	},
	{ "grep",		STRING,		{0}
	  DOCSTR("The command to use for the Zedit grep command. The default "
		 "is `grep -n'.")
	},
	{ "kill-line",		FLAG,		{1}
	  DOCSTR("If this variable is set, the 'Delete to End of Line' command "
		 "will delete the entire line. If at the start of line, it "
		 "will delete the line including the NL. The default is on.")
	},
	{ "lines",		FLAG,		{0}
	  DOCSTR("This flag variable turns on a line:column indicator on the "
		 "mode line. The Lines feature can cause a lot of cursor "
		 "movement and is irritating on some slow terminals. The "
		 "default is off.")
	},
	{ "make-cmd",		STRING,		{0}
	  DOCSTR("This is a string variable which contains the default string "
		 "for the Make command. The default is 'make'.")
	},
	{ "margin",		DECIMAL,	{0}
	  DOCSTR("This is a decimal variable that defines the left margin for "
		 "Text mode and the Format Paragraph command. It does not "
		 "affect the first line of the paragraph. The default is 0. ")
	},
	{ "match",		FLAG,		{1}
	  DOCSTR("This variable controls the C and Shell Mode matching "
	...
	{ "normal",		FLAG,		{1}
	  DOCSTR("If on, sets the default buffer mode to Normal. If off, sets "
		 "the default buffer mode to Text. Some file extensions "
		 "override this variable. The default is on.")
	},
	{ "sh-extends",		STRING,		{0}
	  DOCSTR("This variable defines up to 8 extensions that turn on Shell "
		 "Mode. The extensions are seperated with the colon (:).")
	},
	{ "silent",		FLAG,		{0}
	  DOCSTR("If set, turns the bell off. Default is off (bell on).")
	},
	{ "single-scroll",	FLAG,		{0}
	  DOCSTR("If SingleScroll is set, a Next Line command at the bottom of "
		 "the screen will scroll one line. If not set, the Next Line "
		 "will scroll a few lines. The default is off.")
	},
	{ "space-tabs",		FLAG,		{0}
	  DOCSTR("The tab character is normally inserted into the buffer as a "
	...
	{ "tagfile",		STRING,		{0}
	  DOCSTR("This specifies the tag file name used by the Find Tag "
		 "command. It is a full path name to the tag file. The "
		 "default is empty.")
	},
	{ "t-extends",		STRING,		{0}
	  DOCSTR("This variable defines up to 8 extensions that turn on Text "
		 "Mode. The extensions are seperated with the colon (:). An "
		 "extension of '.' matches no extension (i.e. no '.' in file "
		 "name).")
	},
	{ "visible-bell",	FLAG,		{1}
	  DOCSTR("If this variable is set, the bell will flash the screen "
