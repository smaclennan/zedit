...
Proc Zsetavar()
{
	...
	if(Argp && StartProg("Zvarhelp") == 0) return;
#endif
	...
	if( !Argp || Vars[rc].vtype == STRING )
	{
		...
		if( Getarg(pstr, arg, STRMAX) ) return;
...
#endif
	if( !Vars[VMAIL].val ) Vars[ VMAIL ].val = (Word)strdup("mail");
	if( !Vars[VMAKE].val ) Vars[ VMAKE ].val = (Word)strdup("make");
	if( !Vars[VPRINT].val ) Vars[VPRINT].val = (Word)strdup("lp");
	if( !Vars[VCEXTS].val )
	{
		Vars[VCEXTS].val = (Word)strdup(".c:.h:.cpp:.cc:.cxx:.y:.l:.m:.m4");
...
static void ReadConfigFile(char *fname)
{
	...
	if((fp = fopen(fname, "r")))
	{
		if(Verbose) Dbg("Config file %s\n", fname);
		if(Readstr(buff, fp) && strcmp(buff, "#!m4") == 0)
		{
			fclose(fp);
			sprintf(PawStr, "m4 %s", fname);
			if((fp = popen(PawStr, "r")) == NULL)
			{
				if(Verbose) Dbg("%s failed.\n", PawStr);
				return;
			}
...
void Setavar( vin, display )
char *vin;
Boolean display;
{
	extern char mkcmd[], grepcmd[];
	char *ptr, msg[ STRMAX + 1 ];
	int i = 0;

	strcpy(msg, vin);
	if((ptr = strchr(msg, ' ')) != 0) *ptr = '\0';
	if((ptr = strchr(msg, '\t')) != 0) *ptr = '\0';
	if(Verbose > 1) Dbg("SetAVar '%s' (%s) ", msg, vin);

	for( i = 0; i < NUMVARS; ++i )
#if 0
		if( Strnicmp(vin, Vars[i].vname, 3) == 0 )
#else
		...
		{	/* found a match */
			...
			else
			{
				...
				if( *ptr )
				{
					...
					{
						if(Vars[i].val) free((char *)Vars[i].val);
			...
			if( i == VFONT )
			{
				...
				{
					sprintf(PawStr, "Unknown font %s.", Vars[VFONT].val);
					Error(PawStr);
					return;
				}
			}
			...
			if( i == VFILLWIDTH || i == VMARGIN )
			{
				...
				/* Fillwidth must be > 0 */
				if( Vars[VFILLWIDTH].val == 0 ) Vars[ VFILLWIDTH ].val = 1;
				/* Fillwidth must be greater than Margin */
				if( Vars[VFILLWIDTH].val <= Vars[VMARGIN].val )
				{
					if( i == VMARGIN )
						Vars[ VMARGIN ].val = Vars[ VFILLWIDTH ].val - 1;
					else
						Vars[ VFILLWIDTH ].val = Vars[ VMARGIN ].val + 1;
				}
			}
			else if( i == VMAKE )
			...
			if( display )
			{
				...
				if( Vars[i].vtype == STRING )
					sprintf( msg, "%s = %s", Vars[i].vname,
						Vars[i].val ? (char *)Vars[i].val : "NONE" );
				else
					sprintf( msg, "%s = %d", Vars[i].vname, Vars[i].val );
				Echo( msg );
				if( i == VLINES ) Curwdo->modeflags = INVALID;
			}
			break;
		}
	if(i == NUMVARS)
	{
		if(Verbose > 1) Dbg("no match\n");
		if( display)
		{
...
unsigned mode;
{
	...
	Vars[i].val = abs(Vars[i].val);
	if(Tmaxcol() != EOF)
		if(Vars[i].val > Tmaxcol() - 4) Vars[i].val = Tmaxcol() - 4;
	if(Vars[i].val == 0) Vars[i].val = 1;
