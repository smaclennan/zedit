...
static void blockmove(struct mark *from, struct mark *to)
{
	...
	while (bisbeforemrk(Curbuff, from)) {
		tmp = *Curcptr;
		bswappnt(Curbuff, to);
		binsert(Curbuff, tmp);
		bswappnt(Curbuff, to);
...
{	/* This code must handle any char so that expansion will work */
	...
	case '}':
		...
		Arg = 0;
		tmark = zcreatemrk();
		bmrktopnt(Curbuff, tmark);	/* save current position */
		for (cnt = 0, bmove(Curbuff, -1); !bisstart(Curbuff); bmove(Curbuff, -1))
			if (*Curcptr == '{') {
				if (cnt)
					--cnt;
				else {  /* matched */
				...
				crfound = true;
		bpnttomrk(Curbuff, tmark);	/* no match - go back */
		unmark(tmark);
		tbell();		/* and warn user */
		break;
...
void Zc_indent(void)
{
	...
	if ((Curbuff->bmode & OVERWRITE))
		...
		if (bisaftermrk(Curbuff, &tmark))
			bpnttomrk(Curbuff, &tmark);
		for (width += bgetcol(true, 0); bisbeforemrk(Curbuff, &tmark); bmove1(Curbuff))
			if (*Curcptr == '{') {
...
{	/* Not reentrant - must iterate for arg */
	...
	do {
		...
		/* main loop */
		while (bisbeforemrk(Curbuff, tmp)) {
			...
			movepast(biswhite, FORWARD);
			if (*Curcptr == NL && bisbeforemrk(Curbuff, tmp)) {
				bdelete(Curbuff, 1);
				Ztrim_white_space();
				binsert(Curbuff, ' ');
			}
	...
	bpnttomrk(Curbuff, tmark); /* restore point */
	unmark(tmark);
}
...
void Zcount(void)
{
	...
	putpaw("Counting...");
	word = false;
	for (; Argp ? bisbeforemrk(Curbuff, UMARK) : !bisend(Curbuff); bmove1(Curbuff), ++c) {
		if (ISNL(*Curcptr))
			++l;
		if (!bistoken())
			word = false;
...
static void setregion(int (*convert)(int))
{
	...
	for (; bisbeforemrk(Curbuff, UMARK); bmove1(Curbuff))
