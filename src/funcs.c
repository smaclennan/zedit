...
#ifdef CMDPROF
static unsigned long cmdcnt[NUMFUNCS + '~' - ' ' + 1];

void CMDinit()
{
	memset(cmdcnt, '\0', sizeof(unsigned long) * NUMFUNCS);
	CMDreadwrite(READ_MODE);
}


void CMDreadwrite(mode)
int mode;
{
	char path[PATHMAX + 1];
	int fd, n;
	extern struct passwd *Me;
	sprintf(path, "%s/.cmdcnt.z", Me->pw_dir );
	if((fd = open(path, mode, 0666)) != EOF)
	{
		switch(mode)
		{
		case READ_MODE:  n =  read(fd, (char *)cmdcnt, sizeof(cmdcnt)); break;
		case WRITE_MODE: n = write(fd, (char *)cmdcnt, sizeof(cmdcnt)); break;
		}
		close(fd);
	}
}
...
void CMDinsert(cmd, key)
int cmd, key;
{
	++cmdcnt[cmd];
	if(key >= ' ' && key <= '~') ++cmdcnt[key - ' ' + NUMFUNCS];
}
#endif
